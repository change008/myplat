...
namespace myplat.Util
{
    public class HTMLUtil
    {
        ...
        /// <param name="htmlString"></param>
        /// <returns></returns>
        public static string ClearImg(string htmlString, params string[] needClear)
        {
            if (needClear == null)
                return htmlString;
            foreach (string s in needClear)
            {
                Regex reg = new Regex(@"<img.+?\.+sinaimg.cn[^>]*>", RegexOptions.IgnoreCase);
                htmlString = Regex.Replace(htmlString, @"<img.+?\.+sinaimg.cn[^>]*>", "");
            }
            return htmlString;
        }
        ...
        {
            //先去掉等号两边的空格
            htmlnode = Regex.Replace(htmlnode, @"\s*=\s*", "=", RegexOptions.Singleline | RegexOptions.Singleline);
